"""Adam as a plastax optimizer bundle.
...
the optax.adam parity is exact; for sparse regrowth later, a per-edge counter
also restarts a regrown edge's schedule, which is the intended RigL behavior.
...
class _AdamUpdateConn:
    """UpdateConn applying one Adam step per connection.
    ...
    def incoming(
        ...
        update = (
            jnp.float32(self.lr) * m_hat / (jnp.sqrt(v_hat) + jnp.float32(self.eps))
        )
        weight = c[WEIGHT, cid] - update
        return ConnWrite.of((WEIGHT, weight), (self.m, m), (self.v, v), (self.t, step))
...
@dataclasses.dataclass(frozen=True)
class Adam:
    """Adam optimizer bundle with per-connection moments and step counter.
    ...
    is False); the step count is the per-connection ``opt/t`` column.
    ...
    def update_conn(self) -> _AdamUpdateConn:
        """Build the UpdateConn that applies this Adam rule.

        Returns:
            An UpdateConn advancing each connection's moments, step, and weight.
        """
        return _AdamUpdateConn(
            self.lr, self.b1, self.b2, self.eps, self.grad_field, _M, _V, _T
        )
